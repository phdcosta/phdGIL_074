...
phdShapeBase::phdShapeBase() {
	...
	normalColor.set(ofColor(128,128,128,20), ofColor(255,0,0,255), ofColor(255,0,255,255), ofColor(128,128,255,255));
	selectedColor.set(ofColor(255,255,255,20), ofColor(255,0,255,255), ofColor(255,128,0,255), ofColor(255,255,255,255));
	selectedVertex = -1;
}
...
void phdShapeBase::setVertexPos(int _index, double _x, double _y) {
...
void phdShapeBase::setSelectedVertex(int _index) {
	if(_index > -2 && _index < vertices.size())	selectedVertex = _index;
}
...
void phdShapeBase::onMouseMoved(ofMouseEventArgs& args) {

}

void phdShapeBase::onMousePressed(ofMouseEventArgs& args) {
	...
	if(args.button == 0) { // left button
		if(hitTestInfo.operation == goVertex) {
			setSelectedVertex(hitTestInfo.index);
		}
	}
}

void phdShapeBase::onMouseDragged(ofMouseEventArgs& args) {
	...
	if(args.button == 0) { // left button
		if(hitTestInfo.operation == goWidth) {
			...
			setVertexPos(hitTestInfo.index, args.x, args.y);
...
void phdShapeBase::onMouseReleased(ofMouseEventArgs& args) {
	if(hitTestInfo.operation == goWidth) {
...
void phdShapeBase::drawHitTestInfo(ofPolyline & _coords, ofMatrix4x4 & _mat, bool _gimbal, bool _edge, bool _vertex) {
	...
	if(_vertex) {// && canMoveVertex) {
		for(int i = 0; i < _coords.size(); i++) {
			if((hitTestInfo.info == htiVertex && hitTestInfo.index == i) || selectedVertex == i) {
