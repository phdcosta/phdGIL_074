...
void phdBlend::draw(int _blendMode, ofTexture & _tex0, float _factor0, ofTexture & _tex1, float _factor1, float _w, float _h) {
	shader.begin();
	shader.setUniformTexture("base", _tex0, 0);
	shader.setUniformTexture("blendTgt", _tex1, 1);
//	shader.setUniform1i("blendTgt", 0);
	shader.setUniform1i("mode", 5);//_blendMode);        
	shader.setUniform1f("factor0", _factor0);
	shader.setUniform1f("factor1", _factor1);
	ofEnableAlphaBlending();
	ofSetColor(255,255,255,255);
	_tex0.draw(0, 0);
	//ofRect(0,0,_tex0.getWidth(),_tex0.getHeight());
	shader.end();        
}

void phdBlend::loadShader() {
	string psBlendFragShader = "#version 120\n \
        ...
        ";
        shader.setupShaderFromSource(GL_FRAGMENT_SHADER, psBlendFragShader);
        shader.linkProgram();
}