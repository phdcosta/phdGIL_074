...
void phdWarper::warpPoint(float srcX, float srcY, float& dstX, float& dstY) {

	if(changed) { calcWarpMatrix(); }

	GLfloat z = 0.0;
	GLfloat r0 = 0.0; GLfloat r1 = 0.0; GLfloat r2 = 0.0; GLfloat r3 = 0.0;
	r0 = (float)(srcX * glWarpMatrix[0] + srcY*glWarpMatrix[4] + z*glWarpMatrix[8]  + 1.0*glWarpMatrix[12]);
	r1 = (float)(srcX * glWarpMatrix[1] + srcY*glWarpMatrix[5] + z*glWarpMatrix[9]  + 1.0*glWarpMatrix[13]);
	r2 = (float)(srcX * glWarpMatrix[2] + srcY*glWarpMatrix[6] + z*glWarpMatrix[10] + 1.0*glWarpMatrix[14]);
	r3 = (float)(srcX * glWarpMatrix[3] + srcY*glWarpMatrix[7] + z*glWarpMatrix[11] + 1.0*glWarpMatrix[15]);
	dstX = (float)(r0 / r3);
	dstY = (float)(r1 / r3);
}
...
void phdWarper::resetWarpMatrix() {
	...
	for(int i = 0; i < 16; i++) {
		if(i % 5 != 0) glWarpMatrix[i] = 0.0;
		else glWarpMatrix[i] = 1.0;
	}
}
...
void phdWarper::getBounds(double &_x, double & _y, double & _w, double & _h) {
...
void phdWarper::setSize(double _w, double _h) {
	width = _w;
	height = _h;
}

void phdWarper::getSize(double & _w, double & _h) {
	_w = width;
	_h = height;
}
