...
void main( void ) {
	float waveHeight = USE_MOUSE ? (mouse.x / resolution.x) * 5.0 : cos(time * 0.03) * 1.2 + 1.6;
	waveHeight1 *= waveHeight;
	waveHeight2 *= waveHeight;
	waveHeight3 *= waveHeight;
	
	vec2 position = vec2((gl_FragCoord.x - resolution.x / 2.0) / resolution.y, -1.*(gl_FragCoord.y - resolution.y / 2.0) / resolution.y);
	...
	float middayperc = USE_MOUSE ? -1*((mouse.y / resolution.y) * 0.3 - 0.15) : max(0.0, sin(subtime));
