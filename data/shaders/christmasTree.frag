...
void main(void){
  vec2 vPos=-1.0+2.0*gl_FragCoord.xy/resolution.xy; vPos.y = -1*vPos.y;
 
  //Camera animation
  vec3 vuv=normalize(vec3(0,2.,0));
  vec3 vrp=vec3( 0, 3.5, 0);
  vec3 prp=vec3( 3.5, 5.5, 5.5);
  float vpd=2.5;  
  ...
  c=0.4/length(xy);
  pc+=vec4(
          (sin(i*5.0+time*1.0)*0.5+0.5)*c,
          (cos(i*2.0+time*8.0)*0.5+0.5)*c,
          (sin(i*1.0+time*12.0)*0.5+0.5)*c,0.0);
  }
  pc=pc/maxl;
  ...
  if (f<maxd){
    if (s.y==0.0)
      ...
      c=obj2_c(p);
      if (s.y<=1.0){
        gl_FragColor=vec4(c*max(1.0-f*.07,0.3),0.5)+pc*.5;
      } else{
         //tetrahedron normal   
        ...
        gl_FragColor=vec4((b*c+pow(b,8.0))*(1.0-f*.1),0.5)+pc;
      }
  }
