...
void main( void )
{
	...
	vec3 eye = vec3(Circle(time / 10.0 + 10.0 * mouse.x / resolution.x) * (7.1 - 4.5 * mouse.y / resolution.y), 4.5 * mouse.y / resolution.y);
	...
	gl_FragColor = vec4 (color, 1.0)*mod(gl_FragCoord.y, 2.0)*mod(gl_FragCoord.x, 2.0);;
}