...
void main( void ) {
	...
	for(int i = 1; i <= 8; i++)
	{
		...
		color1 -= ( (1.5 / power) * snoise( vec3( (atan(p.y,p.x))*power, (2.*length(p)-(time/1.))*power,  (time/8.) ) ) );
		color2 -= ( (1.5 / power) * snoise( vec3( (atan(p.y,-p.x)+2.*PI)*power, (2.*length(p)-(time/1.))*power,  (time/8.) ) ) );
	}
